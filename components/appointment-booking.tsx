...
export function AppointmentBooking() {
  ...
  const fetchTherapists = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const counselorsData = data.counselors || data.users || [];
        if (counselorsData.length > 0) {
          const mappedCounselors: Counselor[] = counselorsData.map(
            (u: any) => ({
              id: u.id,
              name: u.name,
              title: u.therapist_profiles?.license_number
                ? "Licensed Counselor"
                : "Therapist",
              specialties: u.therapist_profiles?.specialties || [
                "General Counseling",
              ],
              availability: [
                "Monday",
              ...
              image: u.therapist_profiles?.photo_url || "/placeholder.svg",
              rating: u.therapist_profiles?.rating || 4.5,
              experience: u.therapist_profiles?.years_experience || "5 years",
            }),
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("/api/student/appointments-crud", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          therapistId: selectedCounselor,
          date: selectedDate,
          ...
          notes: `
            ...
            Previous Counseling: ${formData.previousCounseling}
          `.trim(),
        }),
      });
      ...
      <Card className="max-w-2xl mx-auto">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-muted-foreground mb-6">
            ...
            confirmation email with all the details and instructions for your
            session.
          </p>
      ...
      <div className="flex items-center justify-center space-x-2 sm:space-x-4 overflow-x-auto pb-2">
        {[1, 2, 3, 4].map((stepNumber) => (
          <div key={stepNumber} className="flex items-center flex-shrink-0">
            ...
            {stepNumber < 4 && (
              <div
                className={`w-8 sm:w-12 h-0.5 ${step > stepNumber ? "bg-primary" : "bg-muted"}`}
              />
            )}
          </div>
        ))}
      ...
      {step === 1 && (
        <div>
          ...
          <div className="flex justify-center mt-6 sm:mt-8">
            <Button
              onClick={() => setStep(2)}
              disabled={!selectedCounselor || isFetchingTherapists}
          ...
          <Card className="max-w-2xl mx-auto">
            <CardHeader>
              ...
              <CardDescription className="text-sm sm:text-base">
                All information is kept strictly confidential and secure. This
            ...
            <CardContent className="space-y-6">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="high">
                      High - Significant distress or impairment
                    </SelectItem>
              ...
              <div>
                <Label htmlFor="previousCounseling">
                ...
                <Alert className="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/20">
                  <AlertDescription className="text-red-800 dark:text-red-200">
                    ...
                    <p className="text-sm mb-2">
                      ...
                      self-harm, please contact:
                    </p>
                    <div className="text-sm space-y-1">
                      <div>• 24x7 Emergency Helpline: 112</div>
                      <div>• iCall (TISS): 9152987821 (24x7)</div>
                      <div>• Vandrevala Foundation: 1860 2662 345 (24x7)</div>
                      <div>• AASRA Suicide Helpline: 9820466726 (24x7)</div>
                    </div>
                  </AlertDescription>
                </Alert>
              )}
